...
class CalenderBuilder(private val dateInfo: Pair<Int, String>) {
    private val calendar = mutableListOf<Pair<String, Boolean>>() // 날짜 정보와 휴일 여부
    ...
    private fun initDayQueue(targetDay: String) {
        while (true) {
            if (dayQueue.peek() == targetDay) break // peek()
            rotateQueue()
        }
    }
    ...
    private fun setHoliday(date: Int, month: Month) {
        ...
        when(isHoliday(month, date, todayDay)) {
            true -> {
                ...
                calendar.add(Pair(CALENDER_INFO_TEMPLATE.format(month.value, date, todayDay), true))
            }
            false -> calendar.add(Pair(CALENDER_INFO_TEMPLATE.format(month.value, date, todayDay), false))
        }
        rotateQueue()
    }

    private fun isHoliday(month: Month, date: Int, todayDay: String):Boolean {
        return (month.hoiliday.contains(date) || listOf(SATURDAY, SUNDAY).contains(todayDay))
    }
